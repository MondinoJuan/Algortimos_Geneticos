...
def conversor_cultivo_a_entero(df, cultivo_a_entero=None):
    ...
    if cultivo_a_entero is not None:
...
def guardar_modelo(modelo, ruta="Modelos_GB/modelo_gbm.pkl"):
...
def entrenar_y_devolver_modelo(head_cant = None):
    ...
    guardar_modelo(GBR2)
    guardar_modelo(cultivo_a_entero, "Modelos_GB/cultivo_a_entero.pkl") # No es un modelo pero necesito guardarlo
...
def main(entradas_para_predecir = None):
    if entradas_para_predecir is None:
        ...
        modelo = joblib.load("Modelos_GB/modelo_gbm.pkl")
        cultivo_a_entero = joblib.load("Modelos_GB/cultivo_a_entero.pkl")
