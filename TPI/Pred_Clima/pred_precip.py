...
def train_neural_network(X, y, epochs=100, learning_rate=0.001):
    ...
    checkpoint = ModelCheckpoint("model/model_precip.keras", save_best_only=True, monitor='val_loss')
...
def data_to_input_and_out(data):
    ...
    for i in range(len(data) - WINDOW_SIZE):
        X.append(data['precipitacion_mm_mes'][i:i + WINDOW_SIZE])
        y.append(data['precipitacion_mm_mes'][i + WINDOW_SIZE])
    return np.array(X), np.array(y)

def train_LSTM_precip(data):
    ...
    model = load_model("model/model_precip.keras")
...
def predecir_proximos_meses(latitud, longitud, WINDOW_SIZE=14, steps=14):
    ...
    ultimos = data['precipitacion_mm_mes'].values[-WINDOW_SIZE:]
    predicciones = []
    model = load_model("model/model_precip.keras")
...
if __name__ == "__main__":
    ...
    print("Predicciones precipitaciones próximos 14 meses:", predicciones_futuras)
