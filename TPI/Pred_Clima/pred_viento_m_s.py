...
def data_to_input_and_out(data):
    ...
    for i in range(len(data) - window_size):
        X.append(data['velocidad_viento_m_s'][i:i + window_size])
        y.append(data['velocidad_viento_m_s'][i + window_size])
...
def train_neural_network(X, y, epochs=100, learning_rate=0.001):
    ...
    checkpoint = ModelCheckpoint("model/model_v_m_s.keras", save_best_only=True, monitor='val_loss')
...
# Predictions
model = load_model("model/model_v_m_s.keras")
